...
import { CheckCircle, Loader2, AlertCircle, CreditCard, ArrowLeft } from "lucide-react"
...
export default function SuccessPage() {
  ...
  useEffect(() => {
    // Get authorization_id and payer_id from URL params
    ...
    } else if (!authId || !payerId) {
      setError("Faltan parámetros necesarios para procesar el pago. Por favor, inténtalo nuevamente.")
    } else {
      setAuthorizationId(authId)
      // Guarda el PayerID si lo necesitas para futuras operaciones
    }
  }, [searchParams])
  ...
  const confirmPurchase = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(`Error al confirmar la compra: ${errorData.detail || response.statusText}`)
      }
    } catch (err) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center p-4">
        <Card className="bg-gray-900/50 backdrop-blur-xl border-gray-700/50 shadow-2xl w-full max-w-md">
          ...
          <CardContent className="text-center space-y-4">
            <p className="text-red-400">Ola xd</p>
